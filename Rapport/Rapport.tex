...
Cette partie s'appuie sur l'ouvrage de référence en la matière \cite{CNES/ONERA/IGN2008}.
...
\begin{figure}[H]
	...
	\end{center}
\end{figure}
Le domaine \emph{optique} est la réunion du domaine du visible et de l'infrarouge (I.R.) \cite{Wikipedia2017d}.
...
Voici un schéma décrivant le fonctionnement d'un capteur de type \emph{whisk broom} \cite{Hillen2009a}:
\begin{figure}[H]
	\begin{center}
...
Voici un schéma décrivant le fonctionnement d'un capteur de type \emph{push broom} \cite{Hillen2009}:
\begin{figure}[H]
	\begin{center}
...
\begin{figure}[H]
	\begin{center}
		...
		\caption{Nadir, Zénith et Horizons \cite{Wikipedia2017a}}
	\end{center}
\end{figure}
...
Voici les caractéristiques radiométriques du satellite Pléiades \cite{Wikipedia2017b}:
\begin{center}
	\begin{tabular}{|c|c|c|}
	\hline
...
Voici un récapitulatif des niveaux de traitements pour une image satellite de la famille SPOT \cite{Space2010}:
\begin{itemize}
...
Cette partie s'appuie en partie sur l'ouvrage \cite{Goodfellow2016}.
...
\begin{figure}[H]
	\begin{center}
		...
		\caption{Dilemme du choix entre deux modèles \cite{Wikipedia2017c}}
	\end{center}
\end{figure}
...
La réflexion derrière la descente de gradient stochastique est que le gradient de l'ensemble est la moyenne des gradients des couples $(x,y)$ \cite{Bottou2010}.
...
Cette partie s'appuie en partie sur les ouvrages de\cite{Goodfellow2016} et \cite{Nielsen2015}.
...
\begin{itemize}
	...
	\item Une fonction d'\emph{activation} $g : \mathbb{R} \rightarrow \mathbb{R}$, \emph{non-linéaire}.
	      	      Des exemples :
	      \begin{itemize}
	      	\item \emph{ReLU} \cite{Glorot2011}, pour \emph{Rectified Linear Unit} : $g(z) = max(0,z)$
...
L'algorithme de rétro-propagation \cite{Werbos1974} est souvent à tort considéré comme étant la méthode d'optimisation utilisée pour minimiser la fonction de perte.
...
Le \emph{Théorème d'Approximation Universelle} \cite{Hornik1991} nous dit qu'il existe un réseau suffisamment "grand" pour avoir la précision souhaitée sur les résultats.
...
Cette régularisation \cite{Ng2004} limite la capacité de modèles, tels que les réseaux de neurones, les régressions linéaires, polynomiales et logistiques, en ajoutant une composante de pénalisation des paramètres $\Omega(\theta)$ à la fonction de perte $J(x,y,\theta)$, tel que
...
Une façon de contourner le problème est de créer artificiellement des données et de les ajouter à l'ensemble d'apprentissage \cite{Tanner1987}.
...
Cela signifie que nous avons atteint un \emph{minimum local} sur les données de test, il convient alors d'arrêter l'apprentissage avant que l'\emph{erreur de généralisation} empire \cite{Prechelt1998}.
\subsubsection{Dropout}
La méthode \emph{Dropout} \cite{Srivastava2014} est une méthode assez particulière de régularisation, qui consiste à entraîner tour à tour des sous-réseaux de notre réseaux de neurones.
\begin{figure}[H]
	\begin{center}
...
Les réseaux de neurones convolutifs \cite{Krizhevsky2012} sont une famille particulière de réseaux de neurones, qui excelle en terme de performance dans le domaine de la vision par ordinateur.
...
La \emph{réduction de la dimensionnalité} \cite{Hinton2006} est un défi majeur en Statistiques et en Apprentissage Machine.
...
Le \emph{pooling} \cite{Krizhevsky2012}, signifiant \emph{mise en commun}, consiste à résumer l'information apportée par les valeurs de sorties proches en une seule valeur.
...
Un des premiers \emph{datasets} d'images pour la classification est le MNIST \cite{LeCun2010}, composé de $70000$ images noir et blanc de $28 \times 28$ pixels, chacune contenant un chiffre manuscrit.
\begin{figure}[H]
	\begin{center}
...
On citera aussi les \emph{datasets} CIFAR-10 \cite{Krizhevsky2009}, CIFAR-100 \cite{Krizhevsky2009a}, et ImageNet \cite{Krizhevsky2012a}.
...
En $2011$, l'erreur de classification était de $25\%$. En $2012$, le réseau de neurones convolutifs AlexNet \cite{Krizhevsky2012}, composé de $8$ couches, atteignait alors $16\%$ d'erreur. En $2014$, GoogLeNet ($22$ couches) \cite{Szegedy2015}, suivi de près par VGG-16 ($16$ couches) \cite{Simonyan2014}, remportait la compétition avec une erreur de $6.7\%$ (respectivement $7.3\%$). En $2015$, ResNet \cite{He2015}, un réseau de $152$ couches, atteignait alors $3.6\%$. En moyenne, un humain non-entraîné réalise environ $10\%$ d'erreur et un humain entraîné $5\%$.
...
\subsubsection{Datasets}
Le \emph{dataset} Pascal VOC \cite{Everingham2010} est composé de $11530$ images contenant au total $27450$ régions d'intérêt décrivant $20$ classes.

ImageNet \cite{Krizhevsky2012a} possède aussi une base de détection d'objets contenant environ $450000$ images contenant $475000$ objets référencés répartis en $200$ classes.
\subsubsection{Architectures de Modèles}
Deux architectures remarquables sont les réseaux de type R-CNN \cite{Girshick2013} et YOLO \cite{Redmon2015}, l'une par ses performances exceptionnelles, l'autre par le compromis réalisé entre performances et vitesse d’exécution.
...
\subsubsection{Datasets}
Pascal VOC \cite{Everingham2010} possède aussi une base de segmentation d'images contenant $2913$ images décrivant $20$ classes.
Microsoft COCO (Common Objects in Context) \cite{Lin2014} contient $328000$ images décrivant $91$ classes.
...
\subsubsection{Architectures de Modèles}
Deux architectures remarquables sont les réseaux de type FCN \cite{Long2015} et DeconvNet \cite{Noh2015}, elles utilisent toutes les deux le principe de décomposer l'image en \emph{features} par convolutions successives, puis de reconstituer l'image en image segmentée en appliquant plusieurs couches de déconvolutions.
...
SpaceNet \cite{DigitalGlobe2017} est une base de données d'images satellite très haute résolution pour la détection et segmentation de bâtiments.
...
C'est pourquoi il a été jugé intéressant de compléter les labels originels de SpaceNet par des labels issus d'OpenStreetMap \cite{OpenStreetMap2017}.
...
Nous utiliserons l'API de calcul GPU \emph{CUDA} \cite{NVIDIACorporation2010}, développé par NVIDIA.

\subsubsection{Caffe}
Le \emph{framework} utilisé dans les applications que nous avons développées s'appelle Caffe \cite{Jia2014}.
...
Avant de présenter chacune des expériences et de comparer les modèles utilisés, nous allons d'abord présenter l'architecture ayant inspiré ces modèles, nommée DeconvNet \cite{Noh2015}.
...
Voici le détail de l'architecture de DeconvNet, en omettant les couches de \emph{Batch Normalization} \cite{Ioffe2015} et d'activation ReLU \cite{Glorot2011} présentes après chaque couche de convolution:
\begin{center}
	\begin{tabular}{|c|c|c|c|c|c|}
	\hline 
...
Nous avons donc décider de créer des modèles contenant 5 et 7 blocs, et nous avons remplacé les couches d'activation ReLU par des couches ELU \cite{Clevert2015}, plus performantes.
...
La méthode \emph{Adam} \cite{Kingma2014}, signifiant \emph{Adaptative Moment Estimation}, est une amélioration de la méthode \emph{RMSProp}.\\
