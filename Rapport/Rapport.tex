...
\begin{figure}[H]
	\begin{center}
		\includegraphics[scale=0.4]{Images/Secteurs_Activites.png}
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[scale=0.5]{Images/Matrice_Thales_Services.png}
...
Nous aborderons dans un premier temps le sujet de l'imagerie spatiale, où nous présenterons le fonctionnement général d'un satellite de télédétection, ainsi que les problématiques liées aux images satellite.

Ensuite, nous présenterons les généralités de l'apprentissage machine, suivies par une présentation plus précise de l'apprentissage profond, dans laquelle nous expliquerons en détail le fonctionnement des réseaux de neurones, ainsi que les variantes existantes.

Enfin, nous présenterons les applications réalisées au cours de ce stage permettant d'extraire des informations des images satellite, en utilisant des modèles d'apprentissage profond. Les tâches que nous présenterons sont la segmentation d'images et la recherche de similarités dans les images.
...
Nous n'utiliserons que les données issues de la deuxième édition du SpaceNet Challenge, car les images multispectrales de Rio de Janeiro ont une résolution d'1.84m, car elles n'ont pas été fusionnées avec la bande panchromatique.
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[scale=0.2]{Images/MUL_PAN.png}
...
La labellisation des données a été réalisé grâce à Amazon Mechanical Turk, une plateforme de crowdsourcing spécialisée dans la constitution de \emph{datasets} pour des problèmes d'intelligence artificielle.
...
Les labels de SpaceNet, produit par l'Amazon Mechanical Turk, ne localisent que les bâtiments dans les images. Cependant d'autres informations directement observables depuis les images existent, comme par exemple les routes, où encore les cours d'eau.
...
Cependant ces labels ne sont pas de même qualité que ceux provenant du \emph{dataset} SpaceNet, car ils sont issus d'un travail communautaire, il est donc possible que.
...
L'opération de rasterisation consiste produire, à partir des données vectorielles, ainsi que des dimensions et la résolution de l'image souhaitée, une image matricielle où chaque pixel prend une valeur selon si il est contenu ou non dans un polygone.
\begin{figure}[H]
	\begin{center}
		\includegraphics[scale=0.3]{Images/MUL_PAN_Label_Vegas.png}
		\caption{Image et Masque de Segmentation}
	\end{center}
\end{figure}
