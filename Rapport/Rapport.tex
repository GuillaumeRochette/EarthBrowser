...
Thales Services est implanté sur tout le territoire français, chaque site ayant des spécialisations différentes.
\begin{figure}[H]
	\begin{center}
...
Jusqu'aux années $1970$, la télédétection optique servait majoritairement à l'espionnage et la cartographie, principalement réalisée par le biais d'appareils photographies emporté dans des avions.
...
L'apparition de capteurs numériques a révolutionné le domaine de la télédétection, car permettant de s'abstraire du support physique, problématique de par sa quantité limité à bord, ainsi que sa réception.
...
Puis nous aborderons les différents mécanismes relatif au contrôle de l'orbite du satellite
\subsection{Capteurs Optiques}
Selon leur longueur d'onde, les ondes électromagnétique portent des noms différents
\begin{figure}[H]
	\begin{center}
		\includegraphics[scale=0.5]{Images/Domaines_du_spectre_electromagnetique.png}
	\end{center}
\end{figure}
...
Par ailleurs, nous nous focaliserons uniquement sur les capteurs passifs, c'est à dire des capteurs qui ne font que mesurer le rayonnement d'un objet, contrairement aux capteurs actifs, qui émettent un rayonnement et en observent l'écho sur l'objet.
...
\end{itemize}
Néanmoins un tel capteur présente aussi des inconvénients :
\begin{itemize}
...
\end{itemize}
Un inconvénient majeur de ce système est à cause du nombre important de détecteurs, l'étalonnage radiométrique est assez complexe.

\subsubsection{Capteurs Matriciels}


\subsection{L'Orbite des Satellites}
\subsection{Contrôle du Satellite}
\section{Images Satellite}
\subsection{Mosaïque}
\subsection{Fusion d'Images}
\subsection{Angles}
...
Nous avons défini précédemment la convolution entre deux images, ici une \emph{map de features} $I$ et un noyau $K$, de la façon suivante :
...
Soit $I$ un tenseur de dimensions $(H_I, W_I)$ et $K$ un noyau de dimensions $(H_K, W_K)$,\\
...
\begin{itemize}
	\item $l \in [0, C_K^{in}[$
	\item $m \in [0, H_K[$ et $(m + i \times s_H) \in [0, H_I[$
	\item $n \in [0, W_K[$ et $(n + j \times s_W) \in [0, W_I[$
\end{itemize}
...
On remarque qu'on peut transformer le tenseur d'entrée en un tenseur de dimension $(3 \times 256 \times 256) = (196608)$.
...
\end{itemize}
Tout d'abord la première question n'a pas vraiment de sens dans de nombreux domaines, et particulièrement en \emph{vision par ordinateur}, car les pixels d'une image sont liés spatialement. Ils sont donc dépendants de leurs voisins.
...
Nous allons lister deux transformations très efficaces appliquées aux réseaux de neurones, en particulier convolutifs.
