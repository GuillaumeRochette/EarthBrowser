...
Nous avons effectué plusieurs expériences parmi lesquels nous souhaitons présenter les deux ayant produit les résultats les plus intéressants.

La première exploite exploite toutes les données des quatre zones géographiques, c'est-à-dire Las Vegas, Paris, Shanghai et Khartoum, et utilise uniquement les labels de SpaceNet.
La segmentation réalisée est donc binaire, car cela revient à prédire pour chaque pixel de l'image si il fait partie d'un bâtiment ou non.

La seconde expérience exploite uniquement les données de la région de Las Vegas, mais utilise des labels provenant de SpaceNet et d'OpenStreetMap.
La segmentation sera multi-classes, il faudra en effet prédire pour chaque pixel de l'image si il fait partie d'un bâtiment, d'une route ou bien d'aucune de ces deux classes.
